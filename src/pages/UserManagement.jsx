      ...
      <div className="bg-[#1D293D80] border-2 border-[#2B7FFF33] rounded-2xl p-6">
        ...
        <div className="space-y-4">
          {users.map((user) => (
            < div
              key={user.id}
              className="bg-[#0F172B60] border border-[#2B7FFF15] rounded-xl p-4 grid grid-cols-4 items-center gap-4"
            >
              {/* 1️⃣ Profile + Name + Email + Role */}
              ...
              {/* 2️⃣ Store */}
              <div className="text-[#90A1B9]">Store: <br /> <strong className="text-white">  {stores.find(s => String(s.id) === String(user.store))?.name || "—"}</strong></div>

              {/* 3️⃣ Last Active / Status */}
              <div className="text-[#90A1B9]">
                Active Status: <br />
                <strong className="text-white">{user.lastActive}</strong></div>

              {/* 4️⃣ Delete Button (only Super Admin can see) */}
